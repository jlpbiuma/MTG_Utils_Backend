...
async def get_card_details(
    ...
    if card:
        ...
        try:
            where_cond = (
                {"OR": [{"catalogId": catalog_id}, {"id": catalog_id}]}
                if catalog_id
                else {"id": card.get("id")}
            )
            ...
            if not printings and card.get("id") and card.get("id") != catalog_id:
                printings = await db.cardprinting.find_many(
                    where={"OR": [{"catalogId": card["id"]}, {"id": card["id"]}]},
                    order={"releasedAt": "desc"},
                    include={"set": True},
                )
            ruling_card_ids = [cid for cid in list(dict.fromkeys([catalog_id, card.get("id"), *(item.id for item in printings)])) if cid]
            ...
            card["printings"].append(
                {
                    ...
                    "set_type": getattr(set_obj, "setType", None),
                    "icon_svg_uri": getattr(set_obj, "iconSvgUri", None),
